...
void Tracking::PreintegrateIMU()
{
    ...
    while(true)
    {
        bool bSleep = false;
        {
            ...
            if(!mlQueueImuData.empty())
            {
                ...
                cout.precision(17);
                if(m->t<mCurrentFrame.mpPrevFrame->mTimeStamp-0.001l)//imu第一个数据早于上一帧图像则抛掉
                {
                    mlQueueImuData.pop_front();
                }
                else if(m->t<mCurrentFrame.mTimeStamp-0.001l)//imu第一个数据早于当前帧图像则缓存，然后抛掉
                {
                    mvImuFromLastFrame.push_back(*m);
                    mlQueueImuData.pop_front();
                }
                else//晚于或等于当前帧图像时间的imu数据缓存最后一次然后退出循环
                {
                    mvImuFromLastFrame.push_back(*m);
                    break;
                }
    ...
    const int n = mvImuFromLastFrame.size()-1;//最后一个数是等于或大于当前帧时间的，所以循环次数减1
    ...
    for(int i=0; i<n; i++)
    {
        ...
        if((i==0) && (i<(n-1)))
        {
            float tab = mvImuFromLastFrame[i+1].t-mvImuFromLastFrame[i].t;//相邻imu时间戳差
            float tini = mvImuFromLastFrame[i].t-mCurrentFrame.mpPrevFrame->mTimeStamp;//imu与上一帧图像时间戳差
        ...
        {
            acc = (mvImuFromLastFrame[i].a+mvImuFromLastFrame[i+1].a)*0.5f;//均值，所以算法中应该使用的是中值积分
        ...
        if (!mpImuPreintegratedFromLastKF)//KF = KEY FRAME
            cout << "mpImuPreintegratedFromLastKF does not exist" << endl;
        mpImuPreintegratedFromLastKF->IntegrateNewMeasurement(acc,angVel,tstep);//关键帧间的预积分
        pImuPreintegratedFromLastFrame->IntegrateNewMeasurement(acc,angVel,tstep);//帧间的预积分
    }
